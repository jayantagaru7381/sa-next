...
export function softStyles(theme: Theme, colorKey: ColorKey, options?: StyleOptions): CSSObject {
  ...
  if (colorKey === 'inherit') {
    const base: CSSObject = {
      boxShadow: 'none',
      backgroundColor: varAlpha(theme.vars.palette.grey['500Channel'], theme.vars.opacity?.soft?.bg ?? 0.16),
    };

    const hover: CSSObject = getHoverStyles(options?.hover, {
      backgroundColor: varAlpha(
        theme.vars.palette.grey['500Channel'],
        theme.vars.opacity?.soft?.hoverBg ?? 0.32
      ),
    });
  ...
  if (colorKey === 'white' || colorKey === 'black') {
    const base: CSSObject = {
      ...
      backgroundColor: varAlpha('currentColor', theme.vars.opacity?.soft?.commonBg ?? 0.08),
    };

    const hover: CSSObject = getHoverStyles(options?.hover, {
      backgroundColor: varAlpha('currentColor', theme.vars.opacity?.soft?.commonHoverBg ?? 0.16),
    });

    return { ...base, ...hover };
  }

  // Check if the color palette exists and has the required properties
  const colorPalette = theme.vars.palette[colorKey];
  if (!colorPalette || !colorPalette.mainChannel) {
    console.warn(
      `[softStyles] Color palette for "${colorKey}" is not properly configured. Falling back to default styles.`
    );
    return {
      boxShadow: 'none',
      color: theme.vars.palette.grey[800],
      backgroundColor: varAlpha(theme.vars.palette.grey['500Channel'], theme.vars.opacity?.soft?.bg ?? 0.16),
      '&:hover': {
        backgroundColor: varAlpha(theme.vars.palette.grey['500Channel'], theme.vars.opacity?.soft?.hoverBg ?? 0.32),
      },
    };
  }
  ...
  const base: CSSObject = {
    ...
    backgroundColor: varAlpha(
      colorPalette.mainChannel,
      theme.vars.opacity?.soft?.bg ?? 0.16
    ),
  };
